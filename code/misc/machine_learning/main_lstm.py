...
def encoder(comp):
    ...
    prev = kl.Dense(D1_SIZE // comp, activation='tanh')(inpt)
    prev = kl.LSTM(D1_SIZE // comp)(prev)
    prev = kl.Reshape((D1_SIZE // comp, 1))(prev)
    return keras.Model(inpt, prev)

def decoder(comp):
    inpt = kl.Input((D1_SIZE // comp, 1), name='dec_input')
    prev = kl.LSTM(D1_SIZE // comp)(inpt)
    prev = kl.Dense(D1_SIZE)(prev)
